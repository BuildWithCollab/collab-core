// Hand-written implementation of the log sink factories declared in
// <collab/log.hpp>. Plain TU (not a module unit) so the definitions attach to
// the global module, matching the canonical declarations reached via
// `#include <collab/log.hpp>` from any TU. The same symbols satisfy `import
// collab;` consumers because the module partition re-exports the global-
// module-attached declarations via using-decls.
//
// spdlog is a private link dependency of the static collab library; it is
...
// Console sinks render identifier via display name — "[Collab Net]" reads
// better than the bundle ID on a terminal.
...
// Owns the spdlog registration so dropping the sink also drops its named
// logger from spdlog's global registry — letting `clear_sinks()` reclaim it.
...
public:
    spdlog_sink_base(std::shared_ptr<spdlog::logger> logger, std::string name)
        ...
        logger_->set_level(spdlog::level::trace);  // collab is the gatekeeper
...
// Build the spdlog logger before constructing the base — passing
// `make(name), std::move(name)` directly would race the move against the read.
struct spdlog_pair {
...
// File sinks render identifier via bundle ID — "[com.mrowrpurr.collab-net]"
// is what you want when grepping log files long after the fact.
