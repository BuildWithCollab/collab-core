...
def emit_decls(state: AreaState) -> str:
...
def emit_impl(name: str, rel_include: Path, state: AreaState) -> str:
    ...
    lines.append("#endif")
    lines.append(
        f"[[maybe_unused]] const void* const _emit_{flat}_symbols[] = {{"
    )
    for fn in state.inline_functions:
...
def _flat(rel: Path) -> str:
    """Relative path → C++-identifier-safe form (forward slashes → underscores)."""
