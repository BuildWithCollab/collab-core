...
task("codegen")
    on_run(function ()
        os.exec("python scripts/generate.py include/foo/*.hpp --module-name foo --src-out src")
    end)
    set_menu({
        usage       = "xmake codegen",
        description = "Regenerate per-area decls headers, partition cppms, exports snippets, and impl units."
    })
```
...
Four new files appear in `src/`, alongside the primary `foo.cppm` that you already wrote:
...
**`src/semver.exports.inc`** — the re-export snippet: one `export namespace ns { using ::ns::name; }` block per namespace, naming every top-level entity in the canonical. The partition cppm `#include`s this in its module purview.
...
**`src/semver.cppm`** — the partition cppm scaffold. Includes the decls header in its GMF, declares the partition, then `#include`s the exports snippet in the purview. Four lines, and it doesn't change as the canonical evolves.
...
```
python generate.py <canonical.hpp ...> --module-name <name> --src-out <dir>
```

The canonical headers are positional (one or more paths, typically a glob); both flags are required. No defaults — every project's directory shape is different and the script doesn't guess.

| Argument | Purpose |
|---|---|
| `CANONICAL_HPP...` (positional) | One or more canonical inline-header paths. Wildcards (`*`, `?`, `[`) are expanded in Python, so callers that don't shell-expand (xmake's `os.exec`, Windows cmd) still work. A pattern matching no files is an error. |
| `--module-name` | Module name. Goes into `export module <name>:<part>;`, `module <name>;`, and the first segment of each impl unit's `#include <<name>/<rel>.hpp>` line. Must appear as a directory segment in each canonical's path so the include path can be derived. |
| `--src-out` | Directory to write generated files into. Output mirrors the relative layout under the module-name path segment. |

Missing a required flag → argparse usage + exit 2. Run with `--help` for the full text.
...
The `.exports.inc` snippet (which the partition cppm `#include`s) carries one `using ::ns::name;` per top-level entity inside `export namespace ns { ... }`. Two cases are skipped:
